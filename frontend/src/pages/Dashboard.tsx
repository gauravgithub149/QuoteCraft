...
export const Dashboard: React.FC = () => {
  ...
  const pendingCount = quotes.filter((q) => q.status === 'Sent' || q.status === 'Draft' || q.status === 'Pending').length;
  const overdueQuotes = quotes.filter((q) => q.status === 'Overdue');
  const overdueTotal = overdueQuotes.reduce((acc, curr) => acc + curr.grandTotal, 0);

  const dynamicRevenueData = quotes.length > 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-[#e5e7eb] shadow-2xs hover:shadow-xs transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#4d4632]">Total Revenue</span>
          ...
          <p className="text-[11px] text-[#4d4632]/80 mt-1">{acceptedCount} accepted quote(s)</p>
        </div>

        {/* Metric 2 */}
        <div className="bg-white rounded-2xl p-5 border border-[#e5e7eb] shadow-2xs hover:shadow-xs transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#4d4632]">Total Quotes</span>
          ...
          <p className="text-[11px] text-[#4d4632]/80 mt-1">{acceptedCount} closed & accepted</p>
        </div>

        {/* Metric 3 */}
        <div className="bg-white rounded-2xl p-5 border border-[#e5e7eb] shadow-2xs hover:shadow-xs transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#4d4632]">Total Clients</span>
          ...
          <p className="text-[11px] text-[#4d4632]/80 mt-1">{customers.filter((c) => c.status === 'Active').length} active client(s)</p>
        </div>

        {/* Metric 4 */}
        <div className="bg-white rounded-2xl p-5 border border-[#e5e7eb] shadow-2xs hover:shadow-xs transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#4d4632]">Overdue Payments</span>
            <div className="w-8 h-8 rounded-lg bg-purple-50 text-purple-700 flex items-center justify-center">
              <Clock className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold tracking-tight text-[#1f1b11]">${overdueTotal.toLocaleString()}</span>
            <span className="text-xs font-semibold text-red-600">{overdueQuotes.length} overdue</span>
          </div>
          <p className="text-[11px] text-[#4d4632]/80 mt-1">Pending payment follow-ups</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 border border-[#e5e7eb] shadow-2xs flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-[#1f1b11]">Revenue Overview</h2>
              <p className="text-xs text-[#4d4632]">Historical quotation value trended over time</p>
            </div>
            <select
              value={timeframe}
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dynamicRevenueData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0e7d6" />
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#4d4632' }} axisLine={false} />
                <YAxis
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="truncate">
                      <div className="text-xs font-semibold text-[#1f1b11] truncate">{c.company || c.name}</div>
                      <div className="text-[11px] text-[#4d4632] truncate">{c.name}</div>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-xs font-bold text-[#1f1b11]">${(c.totalValue || 0).toLocaleString()}</div>
                    <div className="text-[10px] text-[#4d4632]">{c.totalQuotesCount || 0} quotes</div>
                  </div>
                </div>
              ))
            )}
      ...
      <div className="bg-white rounded-2xl border border-[#e5e7eb] shadow-2xs overflow-hidden">
        <div className="p-5 border-b border-[#e5e7eb] flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-[#1f1b11]">Recent Quotations</h2>
            <p className="text-xs text-[#4d4632]">Overview of latest created sales quotes</p>
          </div>
          <button
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-[#e5e7eb]">
              {filteredQuotes.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-xs text-[#4d4632]">
                    No quotations found. Create your first quote to get started!
                  </td>
                </tr>
              ) : (
                filteredQuotes.slice(0, 5).map((q) => (
                  <tr key={q.id} className="hover:bg-[#faf9f5] transition-colors">
                    <td className="py-3.5 px-4 font-semibold text-[#1f1b11]">{q.quoteNumber}</td>
                    <td className="py-3.5 px-4">
                      <div className="font-medium text-[#1f1b11]">{q.customerCompany || q.customerName}</div>
                      <div className="text-[11px] text-[#4d4632]">{q.customerName}</div>
                    </td>
